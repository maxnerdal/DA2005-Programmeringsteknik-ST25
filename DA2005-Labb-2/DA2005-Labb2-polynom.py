...
def split_polynomial(poly_str):
    """Splits a polynomial string (input) into a list of multiple strings, one for each term (output)
    ...
    for i,char in enumerate(poly_str): # Loop through the full input string with index
        if char in '+-' and i != 0: # when this statement is true the term is finished
            ...
            term += char
    terms_list.append(term) # Append the last term to the list
    return terms_list
...
def polynomial_to_list(terms_list):
    ...
    max_degree = max(poly_dict.keys())#store the maximum key in a variable
    i = 0#set counter to 0 for new iteration
    while i <= max_degree:#iterate from 0 to the highest key in dict
